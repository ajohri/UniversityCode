...
void BTree<K, V>::insert(const K& key, const V& value)
{
    ...
    {
        BTreeNode* nroot = new BTreeNode(false, order);
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    if(mid_elem_itr != child->elements.end())
    {
            new_right->elements.assign(mid_elem_itr, child->elements.end());
            new_right->elements.erase(new_right->elements.begin());
            mid_elem_itr = child->elements.begin() + mid_elem_idx;
            new_left->elements.erase(mid_elem_itr, child->elements.end());
    }
    
    if(mid_child_itr != child->children.end())
    {
            new_right->children.assign(mid_child_itr, child->children.end());
            mid_child_itr = child->children.begin() + mid_child_idx;
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if(subroot->elements.size() > first_larger_idx && subroot->elements[first_larger_idx] == pair.key)
    {
        return;
    }
    ...
    if(subroot->is_leaf)
    {
        auto current_position = subroot->elements.begin() + first_larger_idx;
        subroot->elements.insert(current_position, pair);
    }
    else
    {
