...
void BinaryTree<T>::printPaths() const
{
	...
	printPaths(root, list_values);
}

template <typename T>
void BinaryTree<T>::printPaths(Node * subRoot, std::vector<T> & list) const
{
	// Base Case
	if(subRoot == NULL)
	{
	...
	{
		cout << "Path: ";
		list.push_back(subRoot->elem);
		for(typename::std::vector<T>::iterator itr = list.begin(); itr != list.end(); itr++)
		{
			cout << *itr << " ";
		}
		cout << endl;
...
int BinaryTree<T>::sumDistances(Node * subRoot, int distance) const
{
	...
	distance++;
	return distance + sumDistances(subRoot->left, distance) + sumDistances(subRoot->right, distance);
}
