...
int scheduler_job_finished(int core_id, int job_number, int time) {
  ...
  job_t* curr_job = (job_t*)(priqueue_poll(&pqueue));
  if(curr_job) {
    if(curr_job->flag!=1) {
      curr_job->flag = 1;
      totalresponse+=(time-curr_job->intime);
    }
    curr_job->timesentered++;
    curr_job->entertime = time;
    cores[core_id].job = curr_job;
    return curr_job->id;
  } else {
