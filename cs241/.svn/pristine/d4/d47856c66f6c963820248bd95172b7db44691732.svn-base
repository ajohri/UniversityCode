...
void print_function() {
    ...
    size_t i;
    printf("Print Function: Number of Rules:%zu\n", num_rules);
...
int is_dep_target(char *dependency) {
	...
	//printf("Is Dependent Target: Queue Size: %zu\n", q_size);
	for(i = 0; i < q_size; i ++) {
		temp = queue_at(&rules, i);
		if(strcmp(temp->target, dependency) == 0) {
			//printf("Is Dependent Target: Target:%s Dependency:%s\n", temp->target, dependency);
			ret = 1;
			break;
		}
	}
