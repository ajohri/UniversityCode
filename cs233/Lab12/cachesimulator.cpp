...
Cache::Block* CacheSimulator::find_block(uint32_t address) const {
  ...
  for(auto & a: blocks){
    if(a->is_valid() && a->get_tag() == extract_tag(address, _cache->get_config())){
      _hits++;
      return a;
    }
  }
...
Cache::Block* CacheSimulator::bring_block_into_cache(uint32_t address) const {
  ...
  for(auto & a: blocks){
    if(!a->is_valid()){
      a->set_tag(extract_tag(address, _cache->get_config()));
      a->read_data_from_memory(_memory);
      a->mark_as_valid();
      a->mark_as_clean();
      return a;
    }
  }
  Cache::Block* lru = blocks.front();
  uint32_t lastUsed = INT_MAX;
  for(auto &a: blocks){
    if(a->get_last_used_time() < lastUsed)
    {
      lastUsed = a->get_last_used_time();
      lru = a;
    }
  }
  if(lru->is_dirty()) lru->write_data_to_memory(_memory);
...
void CacheSimulator::write_access(uint32_t address, uint32_t word) const {
  ...
  Cache::Block* block = find_block(address);
  if(block == NULL){
    if(_policy.is_write_allocate()) block = bring_block_into_cache(address);
    else {
  ...
  if(_policy.is_write_back()) block->mark_as_dirty();
