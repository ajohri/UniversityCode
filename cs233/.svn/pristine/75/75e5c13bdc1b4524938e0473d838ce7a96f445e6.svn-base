...
module machine(clk, reset);
   ...
   mips_decode decode(ALUOp, RegWrite, BEQ, ALUSrc, MemRead, MemWrite, MemToReg, RegDst, max, opcode, funct);
   
   regfile rf (rd1_data, rd2_data,
               rs, rt, wr_regnum, final_data, 
               RegWrite, clk, reset);

   mux2v #(32) imm_mux(B_data, rd2_data, imm, ALUSrc);
   alu32 alu(alu_out_data, zero, negative, ALUOp, rd1_data, B_data);
   mux2v #(32) first_mux(final_comparison, max,rd1_data, rd2_data);
   
   data_mem data_memory(load_data, alu_out_data, rd2_data, MemRead, MemWrite, clk, reset);
   
   mux2v #(32) wb_mux(wr_data, alu_out_data, load_data, MemToReg);
   mux2v #(32) final_mux( final_data, max, wr_data, final_comparison);
